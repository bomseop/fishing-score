...
버튼은 **설정값이 아니라 실제 상태**를 말한다. `ON` 은 락을 실제로 쥐고 있을
때만이고, 절전 모드 등으로 거부당하면 `불가` 로 바뀐다. 켜둔 줄 알았는데
화면이 꺼지는 것보다는 못 켰다고 말하는 편이 낫다.

### 6-2. GPS 최근접 포인트 — 만들었다가 제거함

앱 실행 시 최근접 포인트를 자동 선택하는 기능을 넣었다가 걷어냈다.

동작하게 만드는 데 네 번의 수정이 필요했다. Geolocation 플러그인은 권한을
스스로 선언하지 않아 앱이 직접 해야 하고, COARSE 만 선언하면 NETWORK
프로바이더가 없는 기기에서 좌표가 영영 안 잡히며, 플러그인이 실패하면
WebView 경로로 물러설 필요가 있었다.

그렇게 고쳐 동작은 했지만, **41개 포인트를 직접 고르면 되는 앱에서 GPS 가
버는 이득이 권한 요청과 그 뒤에 딸린 문제들을 정당화하지 못했다.**
사용자 요청으로 제거했고, 결과적으로 이 앱이 요구하는 권한은 INTERNET 뿐이다.

되살리려면 `@capacitor/geolocation` 을 다시 넣고 매니페스트에 COARSE+FINE 을
선언한 뒤, 플러그인 → WebView 순으로 물러서는 재시도 사슬을 두면 된다.
git 이력에 다 남아 있다.
